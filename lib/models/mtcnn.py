...
from lib.utils.detect_face import detect_face, extract_face
...
class PNet(nn.Module):
    ...
    def __init__(self, state_dict_dir, pretrained=True):
        super().__init__()
        ...
        if pretrained:
            state_dict = torch.load(state_dict_dir + 'pnet.pt')
...
class RNet(nn.Module):
    ...
    def __init__(self, state_dict_dir, pretrained=True):
        ...
        if pretrained:
            state_dict = torch.load(state_dict_dir + 'rnet.pt')
            self.load_state_dict(state_dict)
...
class ONet(nn.Module):
    ...
    def __init__(self, state_dict_dir, pretrained=True):
        ...
        if pretrained:
            state_dict = torch.load(state_dict_dir + 'onet.pt')
            self.load_state_dict(state_dict)
...
class MTCNN(nn.Module):
    ...
    def __init__(
        self, image_size=160, margin=0, min_face_size=25,
        thresholds=[0.6, 0.7, 0.7], factor=0.709, post_process=True,
        select_largest=True, keep_all=False, device=None, state_dict_dir='output/'
    ):
        super().__init__()
        ...
        self.pnet = PNet(state_dict_dir=state_dict_dir)
        self.rnet = RNet(state_dict_dir=state_dict_dir)
        self.onet = ONet(state_dict_dir=state_dict_dir)
