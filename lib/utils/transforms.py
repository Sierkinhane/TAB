...
MATCHED_PARTS = {
    ...
    "300WLP": ([1, 17], [2, 16], [3, 15], [4, 14], [5, 13], [6, 12], [7, 11], [8, 10],
             [18, 27], [19, 26], [20, 25], [21, 24], [22, 23],
             [32, 36], [33, 35],
             [37, 46], [38, 45], [39, 44], [40, 43], [41, 48], [42, 47],
             [49, 55], [50, 54], [51, 53], [62, 64], [61, 65], [68, 66], [59, 57], [60, 56]),
...
def transform_pixel_3d(pt, center, scale, res, z_res, invert=0, rot=0):
    # Transform pixel location to different reference
...
def transform_preds(cfg, coords, center, scale, output_size):

    for p in range(coords.size(0)):
        if cfg.DATASET.DATASET == '300W':
            coords[p, 0:2] = torch.tensor(transform_pixel(coords[p, 0:2], center, scale, output_size, 1, 0))
        else:
            coords[p, 0:2] = torch.tensor(transform_pixel_float(coords[p, 0:2], center, scale, output_size, 1, 0))
    return coords
